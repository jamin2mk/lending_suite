...
final class ProcessClientImpl extends BaseClient implements ProcessClient {
	...
	/**
	 * <p>
	 * Will use only one parameter of processAppId, snapshotId or branchId. Which
	...
	public RestRootEntity<ProcessDetails> startProcess(@Nonnull String bpdId, @Nullable String processAppId,
			@Nullable String snapshotId, @Nullable String branchId, @Nullable Map<String, Object> input) {
	...
	public RestRootEntity<ProcessDetails> countProcessInstanced() {
		
		SafeUriBuilder uri = new SafeUriBuilder(rootUri).addPath(COUNT);

		return makeGet(httpClient, httpContext, uri.build(), new TypeToken<RestRootEntity<ProcessDetails>>() {
		});
	}
}
